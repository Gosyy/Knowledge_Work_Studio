...
## KR-5B validated inspect bundle

KR-5B hardens the KR-5A inspect output by validating the produced bundle as a product artifact.
The validation is profile-neutral and runs in production readiness checks through:

```text
scripts/kw_xlsx_validation_bundle_check.py
```

The bundle validation report is generated by the checker and can also be written next to a sample bundle for inspection.
The runtime bundle itself remains deterministic and non-destructive.
