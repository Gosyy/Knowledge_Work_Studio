...
from backend.app.services.slides_service.live_benchmark_prompt_schema_hardening import (  # noqa: E402
    REQUIRED_RESPONSE_SCHEMA_FIELDS,
    hardened_prompt_for_scenario,
    validate_hardened_response_payload,
)
from backend.app.services.slides_service.live_gigachat_selected_benchmark import PUBLIC_API_DEV_ROUTE, REQUIRED_PROVIDER  # noqa: E402
...
def _read_json_response(request: Request, *, timeout: float) -> dict[str, object]:
    with urlopen(request, timeout=timeout, context=_ssl_context()) as response:  # nosec - explicit operator-configured endpoint
...
def _get_access_token(timeout: float) -> tuple[str, str]:
    ...
    if auth_key and auth_key.strip():
        ...
        raise RuntimeError("S13d hardened live rerun requires GigaChat credentials in shell env.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run S13d hardened live public_api_dev GigaChat rerun for 12 selected benchmark scenarios.")
...
def main() -> int:
    ...
    if token:
        for index, scenario_id in enumerate(S10_SCENARIO_IDS, start=1):
            out_file = artifacts_dir / f"s13d_{index:02d}_{scenario_id}_hardened_gigachat_response.json"
            prompt = hardened_prompt_for_scenario(scenario_id)
            try:
                response = _call_gigachat(prompt, token=token, timeout=timeout, model=model)
                text = _extract_text(response)
                schema_errors: list[str] = []
                parsed_payload: object | None = None
                try:
                    parsed_payload = _parse_jsonish_text(text)
                    schema_errors = validate_hardened_response_payload(parsed_payload, scenario_id)
                except Exception as exc:
                    schema_errors = [f"schema_parse_failed: {type(exc).__name__}: {str(exc)[:160]}"]
                payload = {
                    "scenario_id": scenario_id,
                    "provider": REQUIRED_PROVIDER,
                    "route": PUBLIC_API_DEV_ROUTE,
                    "model": model,
                    "hardened_public_api_dev_execution_performed": True,
                ...
                out_file.write_text(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True, default=str), encoding="utf-8")
                scenario_results.append({k: v for k, v in payload.items() if k not in {"response", "parsed_payload"}})
                if not text.strip():
                    errors.append(f"{scenario_id}: empty GigaChat response text")
                if args.require_schema_valid and schema_errors:
                    errors.append(f"{scenario_id}: schema validation failed: {'; '.join(schema_errors[:4])}")
    ...
    schema_valid_count = sum(1 for item in scenario_results if item.get("schema_valid") is True)
    ...
    manifest = {
        "workflow_id": "slides.live_benchmark_prompt_schema_hardened_rerun",
        "s_phase": "S13d-live",
        ...
        "schema_valid_scenario_count": schema_valid_count,
        "hardened_public_api_dev_execution_performed_by_s13d_live": success_count > 0,
        "credential_input_names_configured": _configured_secret_names(),
        "credential_source": token_source,
        "credential_values_recorded": False,
        "raw_secret_values_recorded": False,
        "server3_local_intranet_route_verified_by_s13d_live": False,
        "public_api_dev_route_is_not_server3_proof": True,
        "completed_human_review_results_present_by_s13d_live": False,
        "auto_approval_allowed_by_s13d_live": False,
        "selected_offline_workflow_parity_claim_supported_now_by_s13d_live": False,
        "kimi_level_claimed_by_s13d_live": False,
    ...
    (artifacts_dir / "s13d_hardened_live_generation_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2, sort_keys=True, default=str), encoding="utf-8")
    if args.json:
        print(json.dumps(manifest, ensure_ascii=False, indent=2, sort_keys=True, default=str))
    else:
        print(f"S13d hardened live public_api_dev GigaChat rerun: {manifest['status']}")
        print(f"Successful scenarios: {success_count}/{len(S10_SCENARIO_IDS)}")
        print(f"Schema-valid scenarios: {schema_valid_count}/{len(S10_SCENARIO_IDS)}")
        for error in errors:
            print(f"- {error}")
