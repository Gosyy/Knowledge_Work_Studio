...
def get_access_token(timeout: float) -> tuple[str, str]:
    ...
    if auth_key and auth_key.strip():
        ...
        raise RuntimeError("S13i single-scenario retry requires GigaChat credentials in shell env.")
    req = Request(
...
def find_s13h_manifest(root: Path) -> Path:
    matches = sorted(root.rglob("s13h_targeted_retry_manifest.json"))
    if not matches:
        matches = sorted(root.rglob("s13h-targeted-retry-report.json"))
    if not matches:
        raise RuntimeError("S13i requires an S13h targeted retry manifest")
    return matches[0]
...
def retry_executive_memo(*, token: str, timeout: float, model: str) -> tuple[dict[str, Any], list[str]]:
    scenario_id = S13I_RETRY_SCENARIO_ID
    prompt = minimal_prompt_for_scenario(scenario_id)
    response = call_gigachat(prompt, token, timeout, model)
    ...
    payload = {
        ...
        "model": model,
        "single_scenario_retry_performed": True,
        ...
        "prompt_digest": json_digest(prompt),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Retry only executive_memo_to_board_deck and merge with prior S13h 11/12 canonical outputs.")
    parser.add_argument("--s13h-live-input", type=Path, required=True)
    ...
    with tempfile.TemporaryDirectory(prefix="s13i-s13h-input-") as tmp:
        input_root = extract_zip(args.s13h_live_input.resolve(), Path(tmp)) if args.s13h_live_input.is_file() else args.s13h_live_input.resolve()
        manifest_path = find_s13h_manifest(input_root)
        source_manifest = load_json(manifest_path)
        if source_manifest.get("route") != PUBLIC_API_DEV_ROUTE:
            raise RuntimeError("S13h manifest route must be public_api_dev")
        if int(source_manifest.get("canonical_schema_valid_scenario_count_after_merge") or 0) < 11:
            raise RuntimeError("S13i requires a prior S13h run with 11 canonical-valid outputs")
        if source_manifest.get("credential_values_recorded") is not False or source_manifest.get("raw_secret_values_recorded") is not False:
            raise RuntimeError("S13h manifest must not record credential values")
        ...
        for index, scenario_id in enumerate(S10_SCENARIO_IDS, start=1):
            out_path = artifacts_dir / f"s13i_{index:02d}_{scenario_id}_merged_canonical_response.json"
            if scenario_id != S13I_RETRY_SCENARIO_ID:
                prior_file = find_scenario_file(input_root, scenario_id)
                if prior_file is None:
                    errors.append(f"{scenario_id}: missing prior S13h response file")
                    continue
                prior = load_json(prior_file)
                if prior.get("canonical_schema_valid") is not True:
                    errors.append(f"{scenario_id}: prior S13h output is not canonical-valid and was not selected for retry")
                prior["s13i_reuse_prior_s13h_canonical_output"] = True
                prior["s13i_source_prior_file"] = prior_file.name
                prior["s13i_source_prior_file_digest"] = digest_file(prior_file)
                ...
                scenario_results.append({
                    "scenario_id": scenario_id,
                    "source": "reused_prior_s13h_canonical_valid_output",
            ...
            if not token:
                ...
                continue
            try:
                retry_payload, retry_errors = retry_executive_memo(token=token, timeout=timeout, model=model)
                out_path.write_text(json.dumps(retry_payload, ensure_ascii=False, indent=2, sort_keys=True, default=str), encoding="utf-8")
                scenario_results.append({
                    "scenario_id": scenario_id,
                    "source": "single_scenario_s13i_live_retry",
                ...
                if retry_errors:
                    errors.append(f"{scenario_id}: single-scenario retry canonical validation failed: {'; '.join(retry_errors[:4])}")
    ...
    retry_valid_count = sum(1 for item in scenario_results if item.get("source") == "single_scenario_s13i_live_retry" and item.get("canonical_schema_valid") is True)
    reused_count = sum(1 for item in scenario_results if item.get("source") == "reused_prior_s13h_canonical_valid_output")

    if args.require_all_canonical_valid and canonical_valid_count != len(S10_SCENARIO_IDS):
        errors.append(f"expected all {len(S10_SCENARIO_IDS)} scenarios canonical-valid after S13i merge, got {canonical_valid_count}")

    manifest = {
        "workflow_id": "slides.single_scenario_executive_memo_retry",
        "s_phase": "S13i-live",
        ...
        "retry_scenario_ids": [S13I_RETRY_SCENARIO_ID],
        "retry_scenario_count": 1,
        "reused_canonical_scenario_count": reused_count,
        "canonical_schema_valid_scenario_count_after_merge": canonical_valid_count,
        "retried_canonical_valid_scenario_count": retry_valid_count,
        "single_scenario_retry_performed_by_s13i_live": retry_valid_count > 0,
        "credential_input_names_configured": configured_secret_names(),
        "credential_source": token_source,
        "credential_values_recorded": False,
        "raw_secret_values_recorded": False,
        "server3_local_intranet_route_verified_by_s13i_live": False,
        "public_api_dev_route_is_not_server3_proof": True,
        "completed_human_review_results_present_by_s13i_live": False,
        "auto_approval_allowed_by_s13i_live": False,
        "selected_offline_workflow_parity_claim_supported_now_by_s13i_live": False,
        "kimi_level_claimed_by_s13i_live": False,
    ...
    (artifacts_dir / "s13i_single_scenario_retry_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2, sort_keys=True, default=str), encoding="utf-8")
    ...
    if args.json:
        ...
        print(f"S13i single-scenario retry: {manifest['status']}")
