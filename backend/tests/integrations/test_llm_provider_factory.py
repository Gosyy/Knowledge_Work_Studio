...
def test_build_llm_provider_supports_fake_test_mode() -> None:
    provider = build_llm_provider(Settings(app_env="test", llm_provider="fake", fake_llm_response="ok"))
...
def test_build_llm_provider_supports_noop_alias() -> None:
    provider = build_llm_provider(Settings(app_env="test", llm_provider="noop", fake_llm_response="noop-ok"))
    assert isinstance(provider, FakeLLMProvider)
...
def test_build_llm_provider_rejects_unknown_provider() -> None:
    with pytest.raises(ValueError, match="Unsupported llm provider"):
        build_llm_provider(Settings(app_env="test", llm_provider="unknown"))


def test_gigachat_provider_requires_credentials_before_runtime_call() -> None:
    provider = build_llm_provider(Settings(app_env="test", llm_provider="gigachat"))
...
def test_factory_unit_cases_are_explicitly_test_scoped() -> None:
    settings = Settings(app_env="test", llm_provider="fake", fake_llm_response="scoped")
