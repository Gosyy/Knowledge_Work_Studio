...
def test_h2_official_execution_flow_persists_execution_run_trace(
    ...
    execution_run_id = executed_task["result_data"]["execution_run_id"]
    ...
    assert execution_runs[0].engine_type == "official_task_execution"
    assert execution_runs[0].status == "succeeded"
    assert execution_runs[0].result_json is not None
    assert execution_runs[0].result_json["task_type"] == "pdf_summary"
    assert execution_runs[0].result_json["output_text"] == "Alpha. Beta."
    assert execution_runs[0].started_at is not None
    assert execution_runs[0].completed_at is not None
